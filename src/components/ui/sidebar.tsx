...
>(
  (
    ...
    const textChild = childNodes.find(node => typeof node === 'string' || (React.isValidElement(node) && node.type === 'span'))
      ...
      >
        {childNodes.map((child, index) => {
            if (typeof child === 'string' || (React.isValidElement(child) && child.type === 'span')) {
                return (
                    <span key={index} className="transition-opacity duration-200 group-data-[state=collapsed]:opacity-0">
                        {child}
                    </span>
                )
            }
